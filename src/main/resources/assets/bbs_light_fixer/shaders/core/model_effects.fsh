...
uniform vec4 ColorModulator, FormColorGrade, GlowingColor, PaintColor;
uniform float FogStart, FogEnd;
uniform vec4 FogColor;
uniform int PassMode;
in float vertexDistance;
in vec4 vertexColor, lightMapColor, overlayColor;
in vec2 texCoord0;
in vec3 formRootPos;
out vec4 fragColor;
uniform mat4 GlowInverse;
uniform float GlowActive, GlowShape;
uniform mat4 PaintInverse;
uniform float PaintActive, PaintShape;
uniform mat4 GradeBrightnessInverse;
uniform float GradeBrightnessActive, GradeBrightnessShape;
uniform mat4 GradeContrastInverse;
uniform float GradeContrastActive, GradeContrastShape;
uniform mat4 GradeSaturationInverse;
uniform float GradeSaturationActive, GradeSaturationShape;
uniform mat4 GradeHueInverse;
uniform float GradeHueActive, GradeHueShape;
...
vec3 bbsApplyFormColorGrade(vec3 rgb, vec3 rootPos)
{
    ...
    {
        float mask = bbsPaintEffectMask(rootPos, GradeBrightnessInverse, GradeBrightnessActive, vec3(0.5), 1.0, GradeBrightnessShape);
    ...
    {
        float mask = bbsPaintEffectMask(rootPos, GradeContrastInverse, GradeContrastActive, vec3(0.5), 1.0, GradeContrastShape);
    ...
    {
        float mask = bbsPaintEffectMask(rootPos, GradeSaturationInverse, GradeSaturationActive, vec3(0.5), 1.0, GradeSaturationShape);
    ...
    {
        float mask = bbsPaintEffectMask(rootPos, GradeHueInverse, GradeHueActive, vec3(0.5), 1.0, GradeHueShape);
        vec3 hsl = bbsRgb2Hsl(clamp(outRgb, 0.0, 1.0));
...
void main() {
 ...
 color.rgb=mix(overlayColor.rgb,color.rgb,overlayColor.a)*lightMapColor.rgb;
 float paintMask=bbsPaintEffectMask(formRootPos,PaintInverse,PaintActive,vec3(.5),1.0,PaintShape);
 if(PaintColor.a>0.001)color.rgb=mix(color.rgb,PaintColor.rgb,clamp(PaintColor.a,0.0,1.0)*paintMask);
 else if(PaintColor.a< -0.001)color.rgb=mix(color.rgb,color.rgb*max(0.0,1.0+PaintColor.a),paintMask);
 float glowMask=bbsPaintEffectMask(formRootPos,GlowInverse,GlowActive,vec3(.5),1.0,GlowShape);
